...
: bindings(), programs(), configLock("__" JucePlugin_Name "ConfigLock__") {
    ...
    config = new PropertiesFile(getDocumentFolder().getChildFile("Settings.xml"),
                                options);
    
    currentSkin =
    config->containsKey("skin") ? config->getValue("skin") : "discoDSP Grey";
    currentBank = "Init";
...
//==============================================================================
int ObxdAudioProcessor::getNumParameters() { return PARAM_COUNT; }
...
void ObxdAudioProcessor::setParameter(int index, float newValue) {
    ...
    if (isHostAutomatedChange) sendChangeMessage();
}
...
//==============================================================================
const String ObxdAudioProcessor::getName() const { return JucePlugin_Name; }
...
bool ObxdAudioProcessor::silenceInProducesSilenceOut() const { return false; }
...
inline void ObxdAudioProcessor::processMidiPerSample(MidiBuffer::Iterator* iter,
                                                     const int samplePos) {
    while (getNextEvent(iter, samplePos)) {
        ...
        if (midiMsg->isController()) {
            ...
            if (bindings.controllers[lastMovedController] > 0) {
                midiControlledParamSet = true;
                setParameter(bindings.controllers[lastMovedController],
                             midiMsg->getControllerValue() / 127.0);
...
bool ObxdAudioProcessor::getNextEvent(MidiBuffer::Iterator* iter,
                                      const int samplePos) {
...
void ObxdAudioProcessor::processBlock(AudioSampleBuffer& buffer,
...
void ObxdAudioProcessor::setStateInformation(const void* data,
                                             int sizeInBytes) {
    if (XmlElement* const xmlState = getXmlFromBinary(data, sizeInBytes)) {
        ...
        if (xprogs->hasTagName(S("programs"))) {
            ...
            forEachXmlChildElement(*xprogs, e) {
                ...
                for (int k = 0; k < PARAM_COUNT; ++k) {
                    programs.programs[i].values[k] =
                    e->getDoubleAttribute(String(k), programs.programs[i].values[k]);
                }
                
                programs.programs[i].name =
...
void ObxdAudioProcessor::setCurrentProgramStateInformation(const void* data,
                                                           int sizeInBytes) {
    if (XmlElement* const e = getXmlFromBinary(data, sizeInBytes)) {
        programs.currentProgramPtr->setDefaultValues();
        
        for (int k = 0; k < PARAM_COUNT; ++k) {
            programs.currentProgramPtr->values[k] = e->getDoubleAttribute(
                                                                          String(k), programs.currentProgramPtr->values[k]);
        }
        
        programs.currentProgramPtr->name =
        e->getStringAttribute(S("programName"), S("Default"));
...
bool ObxdAudioProcessor::loadFromFXBFile(const File& fxbFile) {
    MemoryBlock mb;
    if (!fxbFile.loadFileAsData(mb)) return false;
    ...
    if (compareMagic(set->fxMagic, "FxBk")) {
        ...
        if (fxbSwap(set->numPrograms) >= 0) {
            ...
            const int numParams =
            fxbSwap(((const fxProgram*)(set->programs))->numParams);
            const int progLen =
            (int)sizeof(fxProgram) + (numParams - 1) * (int)sizeof(float);
            
            for (int i = 0; i < fxbSwap(set->numPrograms); ++i) {
                if (i != oldProg) {
                    const fxProgram* const prog =
                    (const fxProgram*)(((const char*)(set->programs)) + i * progLen);
                    if (((const char*)prog) - ((const char*)set) >= (ssize_t)dataSize)
                        return false;
                    
                    if (fxbSwap(set->numPrograms) > 0) setCurrentProgram(i);
                    
                    if (!restoreProgramSettings(prog)) return false;
                }
            }
            
            if (fxbSwap(set->numPrograms) > 0) setCurrentProgram(oldProg);
            
            const fxProgram* const prog =
            (const fxProgram*)(((const char*)(set->programs)) +
                               oldProg * progLen);
            if (((const char*)prog) - ((const char*)set) >= (ssize_t)dataSize)
                return false;
        ...
        if (!compareMagic(prog->chunkMagic, "CcnK")) return false;
        ...
        if ((size_t)fxbSwap(cset->chunkSize) + sizeof(fxChunkSet) - 8 >
            (size_t)dataSize)
            return false;
        ...
        if ((size_t)fxbSwap(cset->chunkSize) + sizeof(fxProgramSet) - 8 >
            (size_t)dataSize)
            return false;
...
File ObxdAudioProcessor::getDocumentFolder() const {
    ...
    .getChildFile("OB-Xd");
    if (folder.isSymbolicLink()) folder = folder.getLinkedTarget();
    return folder;
}
